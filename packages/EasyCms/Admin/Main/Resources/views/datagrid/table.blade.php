<div class="table">
    ...
    @if (isset($results['paginated']) && $results['paginated'])
        @include('admin::datagrid.pagination', ['results' => $results['records']])
    @endif
            ...
            <div class="grid-container">
                <div class="filter-row-one" id="datagrid-filters">
                    <div class="search-filter">
                        <input type="search" id="search-field" class="control" placeholder="{{ __('admin::app.datagrid.search') }}" v-model="searchValue" v-on:keyup.enter="searchCollection(searchValue)" />
                    ...
                    <div class="dropdown-filters">
                        <div class="dropdown-toggle">
                            <div class="grid-dropdown-header">
                                <span class="name">{{ __('admin::app.datagrid.filter') }}</span>
                        ...
                        <div class="dropdown-list dropdown-container" style="display: none;">
                            <ul>
                                <li>
                                    <div class="control-group">
                                        <select class="filter-column-select control" v-model="filterColumn" v-on:click="getColumnOrAlias(filterColumn)">
                                            <option selected disabled>{{ __('admin::app.datagrid.column') }}</option>
                                ...
                                <li v-if='stringConditionSelect'>
                                    <div class="control-group">
                                        <select class="control" v-model="stringCondition">
                                            <option selected disabled>{{ __('admin::app.datagrid.condition') }}</option>
                                            <option value="like">{{ __('admin::app.datagrid.contains') }}</option>
                                            <option value="nlike">{{ __('admin::app.datagrid.ncontains') }}</option>
                                            <option value="eq">{{ __('admin::app.datagrid.equals') }}</option>
                                            <option value="neqs">{{ __('admin::app.datagrid.nequals') }}</option>
                                        </select>
                                    </div>
                                </li>
                                ...
                                <li v-if='stringCondition != null'>
                                    <div class="control-group">
                                        <input type="text" class="control response-string" placeholder="{{ __('admin::app.datagrid.value-here') }}" v-model="stringValue" />
                                    </div>
                                </li>
                                ...
                                <li v-if='numberConditionSelect'>
                                    <div class="control-group">
                                        <select class="control" v-model="numberCondition">
                                            ...
                                            <option value="eq">{{ __('admin::app.datagrid.equals') }}</option>
                                            <option value="neqs">{{ __('admin::app.datagrid.nequals') }}</option>
                                            <option value="gt">{{ __('admin::app.datagrid.greater') }}</option>
                                            <option value="lt">{{ __('admin::app.datagrid.less') }}</option>
                                            <option value="gte">{{ __('admin::app.datagrid.greatere') }}</option>
                                            <option value="lte">{{ __('admin::app.datagrid.lesse') }}</option>
                                        </select>
                                    </div>
                                </li>
                                ...
                                <li v-if='numberCondition != null'>
                                    <div class="control-group">
                                        <input type="number" class="control response-number" placeholder="{{ __('admin::app.datagrid.numeric-value-here') }}"  v-model="numberValue"/>
                                    </div>
                                </li>
                                ...
                                <li v-if='booleanCondition != null'>
                                    <div class="control-group">
                                        <select class="control" v-model="booleanValue">
                                            <option selected disabled>{{ __('admin::app.datagrid.value') }}</option>
                                            <option value="1">{{ __('admin::app.datagrid.true') }}</option>
                                            <option value="0">{{ __('admin::app.datagrid.false') }}</option>
                                        </select>
                                    </div>
                                </li>
                                ...
                                <li v-if='datetimeConditionSelect'>
                                    <div class="control-group">
                                        <select class="control" v-model="datetimeCondition">
                                            ...
                                            <option value="gte">{{ __('admin::app.datagrid.greatere') }}</option>
                                            <option value="lte">{{ __('admin::app.datagrid.lesse') }}</option>
                                            {{-- <option value="btw">{{ __('admin::app.datagrid.between') }}</option> --}}
                                        </select>
                                    </div>
                                </li>
                    ...
                    <div class="dropdown-filters per-page">
                        <div class="control-group">
                            <label class="per-page-label" for="perPage">
                                {{ __('admin::app.datagrid.items-per-page') }}
                            </label>
                ...
                <div class="filter-row-two">
                    <span class="filter-tag" v-if="filters.length > 0" v-for="filter in filters" style="text-transform: capitalize;">
                        ...
                        <span v-else-if="filter.column == 'search'">Поиск</span>
                ...
                <table class="table">
                    @include('admin::datagrid.partials.mass-action-header')

                    @include('admin::datagrid.partials.default-header')

                    @include('admin::datagrid.body', ['records' => $results['records'], 'actions' => $results['actions'], 'index' => $results['index'], 'columns' => $results['columns'],'enableMassActions' => $results['enableMassActions'], 'enableActions' => $results['enableActions'], 'norecords' => $results['norecords']])
                </table>
            </div>
        </script>
                ...
                methods: {
                    ...
                    getResponse: function() {
                        ...
                        if (this.type == 'string') {
                            ...
                            if (this.filterIndex == this.columnOrAlias && (this.numberValue == 0 || this.numberValue < 0)) {
                                ...
                                alert('{{__('admin::app.datagrid.zero-index')}}');
                            }
                    ...
                    formURL: function(column, condition, response, label) {
                        ...
                        if (column == "" || condition == "" || response == "" || column == null || condition == null || response == null) {
                            alert('{{ __('admin::app.datagrid.filter-fields-missing') }}');
                                ...
                                for(colIndex in this.columns) {
                                    if (this.columns[colIndex].index == obj.column) {
                                        ...
                                        if (this.columns[colIndex].type == 'boolean') {
                                            if (obj.val == 1) {
                                                obj.val = '{{ __('admin::app.datagrid.true') }}';
                                            } else {
                                                obj.val = '{{ __('admin::app.datagrid.false') }}';
                                            }
                                        }
                                    }
                                }
                    ...
                    doAction: function(e) {
                        ...
                        if (confirm('{{__('admin::app.datagrid.massaction.delete') }}')) {
