...
@section('page_name')
    <i class="icon angle-left-icon back-link"
       onclick="history.length > 1 ? history.go(-1) : window.location = '{{ route('admin.settings.user') }}';"></i> Изменение данных пользователя {{ $user->name }}
@stop
@section('content')
                        ...
                        <div class="control-group">
                            ...
                            <label class="switch">
                                <input type="checkbox" id="status" name="status"
                                       value="{{ $user->status }}" {{ $user->status ? 'checked' : '' }}>
